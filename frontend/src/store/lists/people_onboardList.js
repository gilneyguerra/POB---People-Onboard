...
export default {
  ...
  actions: {
    async getData({ commit, dispatch }, { page, limit, field, sort }) {
      commit('showLoader');
      try {
        const response = await axios.get(`/people_onboard`, {
    ...
    async getFilteredData({ commit }, request) {
      try {
        commit('showLoader');
        const response = await axios.get(`/people_onboard${request}`);
        commit('getData', response.data.rows);
        commit('hideLoader');
      } catch (e) {
    ...
    async deleteItem({ dispatch, state }) {
      try {
        await axios.delete(`/people_onboard/${state.deleteId}`);
        dispatch('snackbar/showSnackbar', 'People_onboard has been deleted', {
          root: true,
        });
        dispatch('getData');
      } catch (e) {
